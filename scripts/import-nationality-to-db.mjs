...
const sourceUrl = "https://www.pxweb.bfs.admin.ch/api/v1/de/px-x-0102020000_101/px-x-0102020000_101.px";
const sourceCode = "bfs-demographic-balance-by-canton";
...
const response = await fetch(sourceUrl, {
  body: JSON.stringify({
    query: [
      ...
      { code: "Staatsangehörigkeit (Kategorie)", selection: { filter: "item", values: ["2"] } },
...
if (payload.value.length !== cantonIds.length) throw new Error(`Expected 26 foreign-population values, received ${payload.value.length}`);
if (payload.value.some((value) => !Number.isInteger(value) || value < 0)) throw new Error("Received an invalid foreign-population value");
...
try {
  ...
  try {
    ...
    const dataset = await sourceClient.query(`
      ...
      VALUES ($1, 'Bundesamt für Statistik (BFS)', 'Demografische Bilanz nach Kanton', $2, 'Open Government Data (OGD), CC BY 4.0', 'Ständige Wohnbevölkerung nach Staatsangehörigkeitskategorie und Kanton.')
    ...
    const snapshot = await sourceClient.query(`
      ...
      RETURNING id
    `, [dataset.rows[0].id, `${latestYear}-12-31`, contentHash, rawPath, JSON.stringify({ bfsTable: "px-x-0102020000_101", citizenshipCategory: "Ausland", populationComponent: "Bestand am 31. Dezember", expectedCantons: 26 })]);
  ...
  const importRun = await pool.query(`
    ...
    VALUES ($1, 'scripts/import-nationality-to-db.mjs', 'running')
    RETURNING id
  `, [snapshotId]);
  ...
  try {
    ...
    const metric = await importClient.query("SELECT id FROM metric_definition WHERE code = 'population_foreign'");
    ...
    for (const [index, cantonId] of cantonIds.entries()) {
      ...
      `, [geoUnits.get(bfsCantonCodes[cantonId]), metric.rows[0].id, snapshotId, `${latestYear}-12-31`, payload.value[index]]);
    }
  ...
  await pool.query("UPDATE import_run SET status = 'succeeded', completed_at = now(), records_written = $2 WHERE id = $1", [runId, cantonIds.length]);
  console.log(`Imported ${cantonIds.length} latest foreign-population observations for ${latestYear}.`);
} catch (error) {
